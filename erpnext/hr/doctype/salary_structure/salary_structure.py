...
# 		return ret
	...
	def preview_salary_slip(self):
		make_salary_slip(self.name, self.employees[0].employee)
		
		

@frappe.whitelist()
def make_salary_slip(source_name, target_doc=None, employee=None, as_print=False):
	...
	if cint(as_print):
		return frappe.get_print(doc.doctype, doc.name, doc=doc)
	else:
		return doc
